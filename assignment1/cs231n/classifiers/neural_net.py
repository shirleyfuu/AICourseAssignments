...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        scores = np.dot((np.dot(X , W1) + b1).clip(0) , W2) + b2
        ...
        mid = np.maximum(np.dot(X , W1) + b1, 0)
        out = np.exp(scores) / np.exp(scores).sum(1, keepdims=True)
        out[range(N), y] -= 1
        out /= N
        grads['b2'] = out.sum(0)
        grads['W2'] = np.dot(mid.T , out) + reg * W2
        dh = np.dot(out , W2.T)
        dh[mid == 0] = 0
    ...
    def predict(self, X):
        ...
        W1, b1 = self.params['W1'], self.params['b1']
        W2, b2 = self.params['W2'], self.params['b2']
        y_pred = np.argmax(np.dot(np.maximum(X @ W1 + b1, 0) , W2) + b2, 1)
