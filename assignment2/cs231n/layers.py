...
def layernorm_backward(dout, cache):
    ...
    d = dout.shape[1]
    dx = grad_x_normed / std + grad_mu / d + x_normed * std * grad_var * 2 / d
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(output_height):
        for j in range(output_width):
            ...
            dx_slice_flattened = grad_out_slice.dot(w_flat)
            ...
            dw += grad_out_slice.T.dot(x_slice_flattened).reshape(dw.shape)
            db += grad_out_slice.sum(axis=0)
